...
F_entry:
	...
	if (lookahead == '*' ||
		...
		/**/isotimes = lookahead;/**/
		/**/
		if (lookahead != AND && lookahead != NOT && strcmp(op_type, "b") == 0)
		{
			error(ETYPE);
			exit(-2);
		}
	...
	if (lookahead == '+' ||
		...
		/**/isoplus = lookahead;/**/
		/*TYPECHECK*/
		if (lookahead != OR && strcmp(op_type, "b") == 0)
		{
			error(ETYPE);
			exit(-2);
		}
...
void factor(void)
{
	...
	case UINT:
		...
		/**/
		/**/rvalct(lexeme, op_type);/**/
		match(lookahead);

		break;
	case FLTP:
		/*TYPECHECK*/
		/*switch (sizeof(atof(lexeme)))
		{
		case 8:
			strcpy(op, "fl");
			break;
